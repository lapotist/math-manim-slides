...
| `sign_filter` | Which three sign regions need checking? | The negative, zero, and `0<x<1` columns appear without their verdicts. |
| `test_sign_cases` | Where can a nonzero solution live? | Negative `x` has incompatible signs; `0<x<1` has zero left side; `x=0` contributes nothing. |
...
| `solve_corrected` | What final index fits the corrected total? | `N=28` and `c=210/203` are obtained before the parameter is revealed. |
| `derive_corrected_parameter` | Does the corrected variant close consistently? | `t=1/30`, `a=29/900`, and the next candidate lands exactly at 30 and is excluded. |
| `separate_final_results` | Which conclusion belongs to each prompt? | The original 345 and corrected 420 results settle in separate framed statements. |
