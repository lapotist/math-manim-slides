...
## 14 measure_bc_prime (0:20, loop=false)

- Transition: 收起動點路徑，只保留固定的 `ABC`、旋轉端點 `C'` 與 `AC'`。

## 15 derive_endpoint_angle (0:20, loop=false)

- Geometry: 由旋轉得 `angle CAC'=90 degrees`，原正三角形給 `angle BAC=60 degrees`，所以較小的 `angle BAC'=150 degrees`。

## 16 measure_thirty_sixty_triangle (0:30, loop=false)
